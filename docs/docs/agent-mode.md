...
- **Single Router replica for cancel routing.** The in-process invoke registry means `/invoke/cancel` only works against the Router instance that started the invoke. Multi-replica routers (rare today) would need a sticky-session ALB or a real registry; not blocking for v0.3.0.
- **No payload audit by default.** Argv and stdin are intentionally excluded from audit logs to avoid recording secrets. A Helm-flagged `audit.includeInvokePayloads=true` toggle is planned but not in v0.3.0.
- **One reference image.** v0.3.0 ships `sandbox-langgraph` only. Strands+Bedrock, OpenHands, OpenClaw, and an RL-rollout image are deferred to v0.4.0.
